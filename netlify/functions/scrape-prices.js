...
exports.handler = async function(event, context) {
  ...
  // Try to get existing file SHA — works whether file exists or not
  const existing = await githubRequest('GET', `/repos/${GITHUB_REPO}/contents/prices.json`);
  ...
  // If file exists, include SHA for update. If not, omit SHA to create new file.
  if (existing.status === 200 && existing.body.sha) {
    payload.sha = existing.body.sha;
  }

  await githubRequest('PUT', `/repos/${GITHUB_REPO}/contents/prices.json`, payload);
